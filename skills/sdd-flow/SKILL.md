...
**触发词**：实现/新增/修复/删除/重构/开发/fix/implement/refactor/delete/remove 等开发动词，以及「协同开发」「sdd flow」「完整流程」「从规格到归档」。
...
- **S1 → 直接写 + 可选 E2E**：直接实现 + `superpowers:test-driven-development`；非平凡逻辑留一个 `assert` 自检或小测试；不开 openspec、不跑 brainstorm。TDD 完成后，跑 `sdd-gate` 轻量自检（仅第 1 问：是否跳过了 TDD？）；主动询问用户是否需要追加端到端(E2E)测试。
...
| 4 执行 | 子代理三角色 + 强制 TDD + 两阶段 Review | `superpowers:subagent-driven-development` + `superpowers:test-driven-development` + `superpowers:requesting-code-review` + `superpowers:dispatching-parallel-agents`（多 task 并行时） | 通过审查的代码 + 测试 | spec 合规 ✓ 且 质量 ✓ |
| 5 E2E + 归档 | 强制 E2E → 最终验证 → 归档变更 | `superpowers:verification-before-completion` → `openspec-archive` | E2E 通过 + 已归档变更 | E2E ✓ 且 verification ✓ 且 changes 已归档 |
...
4. 调 `superpowers:subagent-driven-development` 执行：每个 task 内强制 `superpowers:test-driven-development`；多 task 可并行时用 `superpowers:dispatching-parallel-agents`；task 完成后触发两阶段 Review（先 spec 合规，后 code quality）
5. 全部通过 + `superpowers:verification-before-completion` 最终验证 + 完整性测试（默认强制进行E2E端到端测试） → 调 `openspec-archive` 归档
...
| 单 task 超时（> 5 分钟无进展） | **自动停下报告用户**，等同连续失败处理，附超时 task 名和当前状态 |
...
- 实际选型：S3 阶段 4 默认 `subagent-driven-development`；若任务仅 1-2 个简单 task、高度耦合需连续操作、或调试探索阶段，改用 `executing-plans` 轻量执行。多 task 可并行时叠加 `dispatching-parallel-agents`。
