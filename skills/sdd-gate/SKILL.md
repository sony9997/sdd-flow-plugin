---
name: sdd-gate
description: SDD 协同开发避坑守门员 —— 检查三大误区（绕过 Superpowers 直接写码 / 混淆 OpenSpec specs 与 Superpowers plans / 全栈 UI 只靠纯文字）。在任何开发阶段都可调用，强制减速对齐。触发词：「避坑」「sdd gate」「检查误区」「跳过了 brainstorm」「specs 和 plans 分不清」「UI 怎么描述」。
license: MIT
metadata:
...
三大误区检查。任何阶段都可调，发现「正在踩坑」就拦下。

触发词：避坑、sdd gate、检查误区、跳过了 brainstorm、specs 和 plans 分不清、UI 怎么描述
